...
export class CoffeeNFT extends ethereum.SmartContract {
  static bind(address: Address): CoffeeNFT {
    return new CoffeeNFT("CoffeeNFT", address);
  }
  ...
  mintNft(buyer: Address): BigInt {
    let result = super.call("mintNft", "mintNft(address):(uint256)", [
  ...
  try_mintNft(buyer: Address): ethereum.CallResult<BigInt> {
    let result = super.tryCall("mintNft", "mintNft(address):(uint256)", [
      ethereum.Value.fromAddress(buyer)
    ]);
    if (result.reverted) {
      return new ethereum.CallResult();
